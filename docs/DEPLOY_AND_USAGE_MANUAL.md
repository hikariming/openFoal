...
2. 商业版（Enterprise Control）：`gateway + web-console + docker-runner + postgres + redis + minio + bootstrap`
...
5. MinIO S3 API：`http://127.0.0.1:19000`
6. MinIO Console：`http://127.0.0.1:19001`
...
期望看到：`gateway / docker-runner / postgres / redis / minio / web-console` 处于 `Up`，`minio-init` 与 `bootstrap-enterprise` 为一次性任务容器（`Exited`）。
...
1. 一键全栈 smoke：`npm run smoke:enterprise:full`
2. 非 Docker 场景联调：`npm run test:p2:e2e`
3. Docker 场景联调：`npm run test:p2:e2e:docker`
4. 企业认证最小验收：
...
9. 企业基础设施端口冲突：通过环境变量改端口后重启（`OPENFOAL_PG_HOST_PORT` / `OPENFOAL_REDIS_HOST_PORT` / `OPENFOAL_MINIO_HOST_PORT` / `OPENFOAL_MINIO_CONSOLE_PORT`）。
